...
import { registerUsageItem } from './usage';

/** Whether `session` was started under one of this window's workspace folders. */
function inThisWorkspace(session: DiscoverableSession): boolean {
  const folders = (vscode.workspace.workspaceFolders ?? []).map((f) => f.uri.fsPath);
  return Boolean(
    session.root && folders.some((f) => session.root === f || session.root!.startsWith(`${f}/`))
  );
}
...
function workingElsewhere(
  ...
  return sessions.filter(
    (s) => s.session_id !== shown.session_id && s.status === 'Running' && inThisWorkspace(s)
  );
}
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  const renderUsage = registerUsageItem(
    context,
    () => ({ usage: core.usage?.(), sessionId: core.activeSession()?.sessionId }),
    () => core.refresh(),
    () => core.backendError()
  );
  ...
  const render = () => {
    ...
    if (!hooksOn) {
      ...
      lines.push(
        'Sessions are adopted, but MoonlightCode cannot tell which one you are',
        'working in.',
        '',
        'Claude Code does not expose a session id to other extensions, so this is only',
        'certain for a session MoonlightCode started or that you adopted from this',
        'panel. Click here to say which it is.'
      );
      ...
      item.command = 'moonlight.sessionControl.adopt';
      lines.push(
        `"${session.title ?? session.session_id}" is NOT adopted, so it is never gated.`,
        'Run "MoonlightCode: Adopt a Claude Code Session".'
      );
    } else {
      ...
      if (busyElsewhere.length > 0) {
        ...
        item.command = 'moonlight.sessionControl.setActive';
        lines.push(
          `Showing "${session.title ?? session.session_id}" (${session.status}), but`,
          `${busyElsewhere.length} other session(s) here are mid-turn:`,
          ...busyElsewhere.map(
            (s) =>
              `  • ${controlApi.sessionLabel(s, core.sessions())} — ${s.status}${
                s.adopted ? '' : ', NOT adopted'
              }`
          ),
          '',
          'The phase above may describe a session you are not talking to. Click to say',
        ...
        lines.push(
          ...
          frozen
            ...
            : `${session.phase} — project writes allowed`,
