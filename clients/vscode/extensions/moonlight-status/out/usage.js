...
exports.registerUsageItem = registerUsageItem;
/**
...
function underPressure(quota, session) {
 ...
 */
function renderText(quota, session) {
...
function renderTooltip(quota, session, sessionKnown, now) {
    const lines = ['Claude usage'];
    if (quota) {
        const resets = quota.five_hour_resets_in
            ? ` — resets in ${quota.five_hour_resets_in}`
            : '';
        lines.push(`5-hour window: ${pct(quota.five_hour_pct)}${resets}`, `Weekly window: ${pct(quota.weekly_pct)}`, `Weekly Sonnet: ${pct(quota.sonnet_pct)}`);
    }
    else {
        lines.push('Account quota unavailable — MoonlightCode could not read your Claude', 'credentials or reach the usage endpoint. The figures are unknown, not zero.');
    }
    lines.push('');
    if (session) {
        const window = session.ctx_limit > 0
            ? `${Math.round(session.ctx_tokens / 1000)}k of ${Math.round(session.ctx_limit / 1000)}k tokens`
            : 'window size unknown';
        lines.push(`Session: ${session.title ?? session.session_id}`, `Model: ${session.model || '—'}${session.persona ? ` · ${session.persona}` : ''}`, `Context: ${pct(session.ctx_pct)} (${window})`, `Uptime: ${session.session_uptime}`);
        ...
        if (age > STALE_MS) {
            lines.push('', `Last observed ${Math.round(age / 60000)}m ago — this session has gone quiet,`, 'so its context and uptime are the last known values, not live ones.');
        }
    }
        ...
        lines.push('No context or uptime for this session: Claude Code reports those through its', 'status line, which MoonlightCode only registers for sessions it starts.');
    }
    else {
 ...
 */
function registerUsageItem(context, read, refresh, backendError) {
    const item = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    item.command = 'moonlight.status.refreshUsage';
    context.subscriptions.push(item, vscode.commands.registerCommand('moonlight.status.refreshUsage', () => refresh()));
    return () => {
