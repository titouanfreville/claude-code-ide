...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('moonlight.sessionControl.adopt', async () => {
        ...
        if (candidates.length === 0) {
            void vscode.window.showInformationMessage(core.sessions().length === 0
        ...
        const picked = await vscode.window.showQuickPick(candidates.map((s) => ({
        ...
        try {
            ...
            await core.refresh();
            void vscode.window.showInformationMessage(`Adopted "${picked.label}".${linked ? ' Linked to this agent panel — the status bar now follows it.' : ''} Gating applies from its next tool call.`);
        }
        catch (err) {
        ...
        if (adopted.length === 0) {
            void vscode.window.showInformationMessage('No adopted sessions — phase only governs sessions under MoonlightCode governance.');
            return;
        }
        ...
        const pickedSession = known
            ...
            : await vscode.window.showQuickPick(adopted.map((s) => ({
                label: controlApi.sessionLabel(s, adopted),
                description: `phase: ${s.phase}${controlApi.FROZEN_PHASES.includes(s.phase) ? ' (project writes DENIED)' : ''}`,
        ...
        const phases = ['Plan', 'AutoImplement', 'Test', 'Review', 'Commit'];
        const pickedPhase = await vscode.window.showQuickPick(phases.map((phase) => ({
            label: phase,
        ...
        try {
            ...
            await core.refresh();
            void vscode.window.showInformationMessage(`"${pickedSession.label}" → ${pickedPhase.label}. The phase is now pinned (a manual pick overrides auto-advance).`);
        }
        catch (err) {
            void vscode.window.showErrorMessage(err instanceof Error ? err.message : String(err));
        }
    ...
    vscode.commands.registerCommand('moonlight.sessionControl.setActive', async () => {
        ...
        const here = all.filter((s) => s.root && folders.some((f) => s.root === f || s.root.startsWith(`${f}/`)));
        ...
        void vscode.window.showInformationMessage(picked.id
            ? linked
        ...
        await vscode.window.withProgress({
            ...
            for (let attempt = 0; attempt < ADOPT_ATTEMPTS; attempt++) {
                ...
                try {
                    ...
                    await core.refresh();
                    void vscode.window.showInformationMessage(`Session ${sessionId.slice(0, 8)} started and adopted — governed from its next tool call, and reviews can be delivered to its terminal.`);
                }
                catch (err) {
                    void vscode.window.showWarningMessage(`Session started, but adopting it failed: ${err instanceof Error ? err.message : String(err)} Use "Adopt a Claude Code Session" to retry.`);
                }
                return;
            }
            ...
            void vscode.window.showWarningMessage(`Session ${sessionId.slice(0, 8)} started, but detection has not seen it yet, so it is NOT governed. It is only detected once it writes its first transcript line — send it a prompt, then run "Adopt a Claude Code Session".`);
        });
    }));
 ...
 */
async function registerSessionsView(context, withCore) {
    const core = await withCore();
    if (!core) {
        return;
    }
    const decorations = new sessions_view_1.SessionDecorations(core);
    ...
    const groups = new groups_store_1.GroupStore(context.workspaceState);
    context.subscriptions.push(groups);
    const governed = new sessions_view_1.SessionsProvider(core, 'governed', groups);
    const unadopted = new sessions_view_1.SessionsProvider(core, 'unadopted', groups);
    context.subscriptions.push(groups.onDidChange(() => {
        governed.refresh();
        unadopted.refresh();
    }), 
     ...
     */
    const updateContext = () => {
        void vscode.commands.executeCommand('setContext', 'moonlight.backendUp', core.backendError() === undefined);
        void vscode.commands.executeCommand('setContext', 'moonlight.hasSessions', core.sessions().length > 0);
    };
    /**
    ...
    const targetOf = (node) => {
        const fromNode = (0, sessions_view_1.nodeSession)(node);
        if (fromNode) {
            return fromNode;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('moonlight.sessions.refresh', () => void core.refresh()), 
    ...
    vscode.commands.registerCommand('moonlight.sessions.addToGroup', async (node) => {
        ...
        const picked = await vscode.window.showQuickPick([...existing.map((g) => ({ label: g.name, id: g.id })), { label: NEW, id: undefined }], { placeHolder: `Add "${session.title ?? session.session_id}" to which group?` });
        if (!picked) {
            return;
        }
        ...
        await ensureCustomMode();
    }), vscode.commands.registerCommand('moonlight.sessions.removeFromGroup', async (node) => {
        const session = targetOf(node);
        if (session) {
            await groups.unassign(session.session_id);
        }
    }), vscode.commands.registerCommand('moonlight.sessions.renameGroup', async (node) => {
     ...
     */
    vscode.commands.registerCommand('moonlight.sessions.deleteGroup', async (node) => {
        const id = groupIdOf(node);
        if (id) {
            await groups.remove(id);
        }
    }), vscode.commands.registerCommand('moonlight.sessions.adopt', async (node) => {
        const session = targetOf(node);
        if (!session) {
        ...
        try {
            ...
            await core.refresh();
            void vscode.window.showInformationMessage(`Adopted ${controlApi.shortId(session.session_id)} — it starts in Plan, where project writes are denied.`);
        }
        catch (err) {
            void vscode.window.showErrorMessage(err instanceof Error ? err.message : String(err));
        }
    }), vscode.commands.registerCommand('moonlight.sessions.setPhase', async (node) => {
        const session = targetOf(node);
        if (!session) {
            return;
        }
        const picked = await vscode.window.showQuickPick(sessions_view_1.PHASES.map((phase) => ({
            label: phase,
            description: controlApi.FROZEN_PHASES.includes(phase)
                ? 'project writes denied'
                : 'project writes allowed',
        ...
        try {
            ...
            await core.refresh();
            void vscode.window.showInformationMessage(`${controlApi.shortId(session.session_id)} → ${picked.label}. The phase is pinned now: a manual pick overrides auto-advance.`);
        }
        catch (err) {
            void vscode.window.showErrorMessage(err instanceof Error ? err.message : String(err));
        }
    }), vscode.commands.registerCommand('moonlight.sessions.advancePhase', async (node) => {
        const session = targetOf(node);
        if (!session) {
            return;
        }
    ...
    }), vscode.commands.registerCommand('moonlight.sessions.setActive', (node) => {
        const session = (0, sessions_view_1.nodeSession)(node);
        if (!session) {
            return;
        }
    ...
    vscode.commands.registerCommand('moonlight.sessions.reviewPlan', async () => {
        ...
        if (!available.includes('moonlight.gate.reviewPlan')) {
            void vscode.window.showWarningMessage('MoonlightCode Agentic Support is not installed, so a held plan cannot be answered from here. Install the MoonlightCode extension pack.');
            return;
        }
