...
exports.ReviewComments = exports.BaselineProvider = exports.BASELINE_SCHEME = void 0;
exports.pickRange = pickRange;
exports.openSessionReview = openSessionReview;
...
exports.BASELINE_SCHEME = 'moonlight-baseline';
/**
 ...
 */
function baselineUri(sessionId, filePath) {
    return vscode.Uri.parse(`${exports.BASELINE_SCHEME}:${filePath}?session=${encodeURIComponent(sessionId)}`);
}
function decodeBaselineUri(uri) {
 ...
 */
function pickRange(threadRange, selection) {
    if (!threadRange) {
        return undefined;
    }
    if (!selection) {
...
function sideOf(uri) {
    if (uri.scheme === exports.BASELINE_SCHEME) {
        return 'Before';
    }
    if (uri.scheme === 'file') {
        return 'After';
...
class ReviewComments {
    ...
    constructor(onChanged) {
        ...
        this.controller = vscode.comments.createCommentController('moonlightcode.review', 'MoonlightCode Review');
     ...
     * session.
     */
    async refresh(sessionId, filePath) {
        ...
        for (const { root, replies } of controlApi.toThreads(stored)) {
            ...
            const uri = root.side === 'Before' ? baselineUri(sessionId, filePath) : vscode.Uri.file(filePath);
            // A file-scoped comment has no range — the native way to say "this is about
            // the file, not a line".
            const range = root.scope === 'File'
                ? undefined
                : new vscode.Range(Math.max(root.start_line - 1, 0), 0, Math.max(root.end_line - 1, 0), 0);
            const thread = this.controller.createCommentThread(uri, range, [
                root,
                ...replies,
            ].map((c) => this.toComment(c)));
            const where = root.scope === 'File'
                ? 'File comment'
    ...
    async create(reply) {
        ...
        if (!sessionId || !side) {
            void vscode.window.showErrorMessage('MoonlightCode: this file is not part of a session review.');
            return;
        }
    ...
    async edit(thread) {
        ...
        const existing = typeof current?.body === 'string' ? current.body : (current?.body.value ?? '');
...
async function openSessionReview(sessionId, title, items, comments) {
    ...
    await vscode.commands.executeCommand('vscode.changes', `Review — ${title} (${items.length} file${items.length === 1 ? '' : 's'})`, resources);
 ...
 */
async function openDiff(item, comments) {
    comments.register(item.session_id, item.file_path);
    ...
    await vscode.commands.executeCommand('vscode.diff', left, right, `${name} (${origin})`, { preview: false });
