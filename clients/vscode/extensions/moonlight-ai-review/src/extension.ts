...
import { deliver, isConfirmed, showHandoff } from './delivery';
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.workspace.registerTextDocumentContentProvider(BASELINE_SCHEME, new BaselineProvider())
  );
  const reviewComments = new ReviewComments(() => void refreshPending());
   ...
   * never existed.
   */
  const reviewCounts = async (): Promise<{ pending: number; resolved: number }> => {
    if (!reviewTarget) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('moonlight.review.addComment', (reply: vscode.CommentReply) =>
      reviewComments.create(reply)
    ),
    vscode.commands.registerCommand(
      'moonlight.review.resolveComment',
      (thread: vscode.CommentThread) => reviewComments.setResolved(thread, true)
    ),
    vscode.commands.registerCommand(
      'moonlight.review.unresolveComment',
      (thread: vscode.CommentThread) => reviewComments.setResolved(thread, false)
    ),
    vscode.commands.registerCommand(
      'moonlight.review.editComment',
      (thread: vscode.CommentThread) => reviewComments.edit(thread)
    ),
    vscode.commands.registerCommand(
      'moonlight.review.deleteComment',
      (thread: vscode.CommentThread) => reviewComments.remove(thread)
    ),
    /**
    ...
    vscode.commands.registerCommand('moonlight.review.toggleResolved', async () => {
      ...
      void vscode.window.showInformationMessage(
        showing ? 'Showing resolved threads.' : 'Hiding resolved threads.'
      );
    }),
    /**
     ...
     * full stop.
     */
    vscode.commands.registerCommand('moonlight.review.commentOnSelection', () =>
     ...
     * a line.
     */
    vscode.commands.registerCommand('moonlight.review.commentOnFile', () =>
      vscode.commands.executeCommand('workbench.action.addComment', { fileComment: true })
    ),
    vscode.commands.registerCommand('moonlight.review.submit', async () => {
      if (!reviewTarget) {
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand('moonlight.review.openSession', async (requested?: string) => {
      const core = await withCore();
      if (!core) {
        return;
      }
      let queue: controlApi.ReviewQueueItem[];
      try {
      ...
      const bySession = new Map<string, controlApi.ReviewQueueItem[]>();
      ...
      let sessionId = resolveReviewSession(
        requested,
        core.activeSession()?.sessionId,
        new Set(bySession.keys())
      );
      if (sessionId === undefined) {
        const picked = await vscode.window.showQuickPick(
          [...bySession.entries()].map(([id, items]) => {
      ...
      if (owner && !controlApi.isReviewable(owner.status)) {
        ...
        const choice = await vscode.window.showWarningMessage(
          `"${owner.title ?? owner.session_id}" is still working (${owner.status}). Comments you write may be about code it is replacing.`,
          go
        );
        if (choice !== go) {
          return;
      ...
      await openSessionReview(
        sessionId,
        items[0].session_title ?? sessionId,
        items,
        reviewComments
      );
      await refreshPending();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('moonlight.review.openQueue', async () => {
      const core = await withCore();
      if (!core) {
        return;
      }
      ...
      panel = vscode.window.createWebviewPanel(
        'moonlightReviewQueue',
        'MoonlightCode Review Queue',
      ...
      current.webview.onDidReceiveMessage(async (message: WebviewMessage) => {
        try {
          switch (message.type) {
            case 'accept':
            ...
            case 'reject': {
              ...
              const outcome = await controlApi.reject(
                message.sessionId,
                message.path,
                message.text
              );
              if (outcome.feedback.status === 'undeliverable') {
                void vscode.window.showWarningMessage(
                  `Rejected — but your feedback was NOT delivered to the session: ${outcome.feedback.reason}`
                );
              } else if (message.text.trim().length > 0) {
                void vscode.window.showInformationMessage(
                  'Rejected — feedback sent to the session.'
                );
              }
              break;
            }
            case 'open': {
              ...
              if (owner && !controlApi.isReviewable(owner.status)) {
                void vscode.window.showWarningMessage(
                  `"${owner.title ?? owner.session_id}" is still working (${owner.status}). Wait until it stops before reviewing — otherwise you are commenting on a moving target.`
                );
                return;
              }
              reviewTarget = message.sessionId;
              const item = (await controlApi.reviewQueue()).find(
                (i) => i.session_id === message.sessionId && i.file_path === message.path
              );
              if (item) {
                await openDiff(item, reviewComments);
              }
              await refreshPending();
              return;
            }
...
type WebviewMessage =
  | { type: 'accept'; sessionId: string; path: string }
  | { type: 'reject'; sessionId: string; path: string; text: string }
  | { type: 'open'; sessionId: string; path: string }
  | { type: 'ignore'; sessionId: string; path: string }
  | { type: 'refresh' };
...
async function submitReview(
  ...
  let outcome: controlApi.SubmitReviewResponse | undefined;
  try {
  ...
  if (outcome.feedback.status === 'queued') {
    ...
    showHandoff(
      context,
      pointer,
      outcome.review_path ?? undefined,
      { kind: 'terminal', detail: 'the session' },
      n
    );
    return;
  }

  const delivery = await deliver(terminals, sessionId, pointer, outcome.review_path ?? undefined);
  showHandoff(context, pointer, outcome.review_path ?? undefined, delivery, n);

  if (isConfirmed(delivery)) {
    try {
      await controlApi.markDelivered(sessionId, outcome.comment_ids);
    } catch {
      void vscode.window.showWarningMessage(
        `Review sent to ${delivery.detail}, but MoonlightCode could not record it as delivered — it may be offered again.`
      );
      return;
    }
    void vscode.window.showInformationMessage(
      `Review delivered — ${n} comment(s) sent to ${delivery.detail}.`
    );
    return;
  }

  void vscode.window.showWarningMessage(
    `Review of ${n} comment(s) saved but NOT delivered — it stays queued. See the hand-off panel to pass it on.`
  );
}

async function renderQueue(panel: vscode.WebviewPanel): Promise<void> {
  let items: controlApi.ReviewQueueItem[] = [];
...
function renderHtml(items: controlApi.ReviewQueueItem[], error: string | undefined): string {
 ...
 */
function badges(item: controlApi.ReviewQueueItem): string {
  const out: string[] = [];
  if (item.created) {
...
function renderItem(item: controlApi.ReviewQueueItem): string {
