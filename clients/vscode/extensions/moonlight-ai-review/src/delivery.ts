...
export type ConfirmedDelivery = Extract<Delivery, { kind: 'terminal' }>;
 ...
 */
export function isConfirmed(d: Delivery): d is ConfirmedDelivery {
 ...
 */
let current: { pointer: string; reviewPath: string | undefined } = {
  pointer: '',
...
export function showHandoff(
  ...
  if (!fallbackPanel) {
    fallbackPanel = vscode.window.createWebviewPanel(
      'moonlightcodeReviewHandoff',
      'MoonlightCode — Review ready',
    ...
    fallbackPanel.webview.onDidReceiveMessage(async (m: { type: string }) => {
