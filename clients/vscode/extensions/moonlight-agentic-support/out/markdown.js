...
function renderMarkdown(markdown) {
    ...
    const withoutFences = source.replace(/```([^\n]*)\n([\s\S]*?)```/g, (_all, lang, code) => {
        fences.push(`<pre class="code"${lang.trim() ? ` data-lang="${escapeHtml(lang.trim())}"` : ''}>` +
            `${escapeHtml(code.replace(/\n$/, ''))}</pre>`);
        return `${MARK}F${fences.length - 1}${MARK}`;
    });
    const escaped = escapeHtml(withoutFences);
    const html = blocks(escaped);
    return html.replace(new RegExp(`${MARK}F(\\d+)${MARK}`, 'g'), (_all, i) => fences[Number(i)] ?? '');
}
...
function inline(text) {
    ...
    const codes = [];
    let out = text.replace(/`([^`]+)`/g, (_all, code) => {
    ...
    !href.startsWith('//') && /^(https?:\/\/|[./#])/.test(href)
        ...
        .replace(/(^|[^*])\*([^*]+)\*/g, '$1<em>$2</em>');
    return out.replace(new RegExp(`${MARK}C(\\d+)${MARK}`, 'g'), (_all, i) => codes[Number(i)] ?? '');
}
