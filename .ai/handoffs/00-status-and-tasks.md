...
**Last updated:** 2026-06-09 (**First usable version committed — `a926f8e`**; the whole stack — engine/control/domain/trust/persistence/mcp-server/desktop + all handoffs & planning docs — landed in the initial usable build. Newest functional slice: the **Stuck/Incomplete session warning signal** (self-report `report_blocked` verb + IDE stall auto-detect → ⚠ on the grid tile). Before that, in order: Git tool window · Commit/Problems/Services tool windows + stripe re-layering · `request_phase` MCP verb · Run console + MCP run verbs.) · **Build:** green (1 cosmetic warning — transitive `block v0.1.6` future-incompat, not our code) · **Tests:** **361 passed / 1 ignored across 21 suites** (verified 2026-06-09; supersedes the old per-crate tally)
**Verify:** `export PATH="$HOME/.cargo/bin:$PATH" && cargo test` · run app via RustRover ▶ "Run moonlight"
**Working tree:** clean of source changes — only `.omc/` session-tracking churn is uncommitted.

### Working rules
- Build/test every slice; report results faithfully. Coordinate on the UI lane (read-before-edit / handoff).

## DISCOVERY MERGED INTO PLAN — 5-phase workflow, uniform `auto` substrate (2026-08-05) — green, 503 tests, clippy 0 errors
Operator: "Discovery and Plan are globally a single phase — keep the Plan naming, but use auto mode in CC with an aim telling it to build a plan and propose it through the IDE panel." Plan doc: `.ai/handoffs/07-merge-discovery-into-plan.md`.
- **Workflow is now `Plan → Auto → Test → Review → Commit → Plan`** (`Phase::ALL` is 5). `Phase::Discovery` is gone; `#[serde(alias = "Discovery")]` on `Plan` migrates persisted rows (a pre-merge session would otherwise come back `StoreError::Corrupt`), and `from_token` still accepts `discovery`/`discover`.
- **Every phase runs CC in `auto`** (`cc_permission_mode()` is constant, `operator_mode()` is always `Mode::Auto`). Consequences: **no phase change ever relaunches CC** (the plan↔auto boundary and `SessionMonitor::relaunch_terminal` are deleted, as is the AGY `/plan` toggle), and `Mode::Plan` survives only as an *observed* state (operator shift-tabbed a live session).
- **The aim replaces CC's native plan behavior.** `moonlight_domain::phase::PLAN_AIM` is the single wording, delivered twice: in the launch system prompt (`mcp_host::ide_context()` + `IDE_CONTEXT_SHORT`, and AGY's `gemini_md()`), and injected into a live session on *entering* Plan (`apply_phase` → `inject_feedback`, `FeedbackOrigin::PhaseAim`) — that covers the mid-conversation switch.
- **Plan keystone = plan approval.** New `Command::ApprovePlan` advances Plan → AutoImplement (pinned ⇒ `PhaseAdvanceRequested`); the plan panel's Approve sends it *after* `ApproveAction` because the composition root swallows an `ApproveAction` that resolved a held hook. Without this an approved plan left the session read-only.
- **Detection reconcile narrowed:** `PhaseObserved(auto)` no longer moves anything (it used to mean "CC left plan ⇒ AutoImplement" — that would now kick every Plan session out on its first transcript line and silently unfreeze writes). Only an observed *native plan mode* still adopts `Phase::Plan`.
- ⚠️ **Runtime-verify:** launch → phase reads **Plan / Auto · plan (read-only)**, the aim lands in the terminal, a project write is denied while `.ai/` is allowed → agent calls `present_plan` → panel → **Approve** → phase flips to **Auto** and the same write succeeds, with no relaunch. Also: open against the existing store and confirm a session persisted as `Discovery` comes back as Plan.

## NEEDS-INPUT TAB BEACON (blinking amber caret) — DONE (2026-06-24) — green, 376 tests, clippy clean (my files)
Operator: surface which sessions need input from the tabs (icon + color, blinking). Design via `/frontend-design`: a **blinking amber input-caret `▮`** (terminal-cursor metaphor = "waiting for you to type"), on-theme with the nocturnal/phosphor aesthetic. **Motion encodes meaning:** blink = NeedsInput ("you, now"); steady dot = a broke/stuck state. Surfaces chosen: **space tabs + dock session tabs**.
- **Shared helper** `panels/mod.rs::needs_input_caret(id)` — amber `▮`, `with_animation(Animation::new(1100ms).repeat().with_easing(pulsating_between(0.25,1.0)), |el,d| el.opacity(d))`. First use of gpui's `Animation` in the app (the element self-drives repaints while visible; stops when removed). `id` keys the animation state → caller seeds it unique (tab index / entity id).
- **Space tabs:** `Workspace::space_attention` now returns `(dot, needs_input)` — the steady dot is the worst kind **excluding** NeedsInput (Stuck/Incomplete/Errored), and `needs_input` is carried by the caret. Fixes the real bug: NeedsInput is the *lowest* `AttentionKind::severity`, so the old single-worst dot **hid** a waiting session behind any error. `SpaceTab` gained `needs_input: bool`; the tab renders the caret after the dot.
- **Dock session tabs:** caret composed into `SessionMonitor::title()` (the per-tab hook), NOT `title_suffix` — verified in gpui-component `tab_panel.rs`: the multi-tab loop renders `panel.title()` per tab, but `title_suffix` renders **only for the active panel** at the far right of the bar. Caret shows when `session.status == WaitingInput`. (Touched `session_monitor.rs` again — other lane; one localized `title()` wrap + the reset fix.)
- ⚠️ **Runtime-verify:** a managed session goes WaitingInput → its dock tab shows a blinking amber `▮` (even when not the active tab), and its space tab shows the caret (alongside a red dot if another session there also errored). Resuming/answering clears it. Dock-tab caret latency: once visible it self-repaints; appearance on transition relies on the dock re-rendering (same as the existing per-tab title/×).

## RESET CRASH FIX + PHASE-AWARENESS (phase_status verb · denial remedy) — DONE (2026-06-23) — green, 374 tests (+2), clippy clean (my files)
Operator: "Reset still break" (live double-lease abort) + "CC often mismatch Phase / moving phase is hard".
- **Reset double-lease crash FIXED** (`session_monitor.rs::reset_session`): the deferred tab-close ran inside `this.update_in` (a `SessionMonitor` lease), but `remove_panel` fires `PanelView::on_removed` on that *same* `SessionMonitor` entity → re-lease → `double_lease_panic` → process abort (and, because it aborted mid-reset after `ForgetSession`, the successor was never persisted → "auto-resume didn't resume on restart"). Fix mirrors `PlanReviewPanel::close_and_return_to_session`: capture `tab_panel` + `cx.entity()` up-front, then close inside `cx.update(|window, cx| close_this_tab(...))` — **no self-lease held** during the removal. Runtime-verify: hit ↻ Reset on a managed session → successor opens, no crash; restart → the session resumes. (Note: `session_monitor.rs` is the other lane's file — edited here because it was a live crash the operator reported directly; one localized fn.)
- **NEW read-only `phase_status` verb** (the mismatch fix): `McpVerb::PhaseStatus` (Safe · `is_read_only` · `min_autonomous_tier=Observed` → ungated, any phase). `PhaseVerbExecutor::phase_status` reads the live phase from the `SessionPolicyView` and reports current phase + mode, project-edit & AI-workspace permissions, what `next` advances to, and the valid targets. Transport tool `phase_status` (no params) + instructions. CC can now orient before acting instead of guessing. +2 tests.
  - (The policy-view wiring + current-aware from→to `request_phase` reply + per-verb phase footer were already present in `phase_verbs.rs` from prior work — I added `phase_status` on top.)
- **Denial remedy hint** (`trust/src/lib.rs`): the write-freeze `Deny` reason now appends the fix — Project freeze → "call request_phase('auto')… (or phase_status…)"; Commit gate → "ask the operator / request_phase off Commit". The agent learns the escape at the moment it's blocked.
- **IDE context** (`mcp_host.rs` `IDE_CONTEXT` + `_SHORT`): now tells sessions to call `phase_status` when unsure and to trust the tool result (not assume) before/after `request_phase`.
- **Transport-drop ROOT CAUSE + FIX (2026-06-23):** the live `request_phase` "transport dropped mid-call" was **not** SSE/keep-alive (rmcp default already pings 15s, stateful) nor a dead server — it was the **unbounded synchronous hold**: a phase-control `Prompt` called `KeystoneApprovalGate::request().await` with `timeout: None` (`control/src/lib.rs:168`), holding the MCP tool call open for the *entire* operator decision; that outlives CC's client tool-call timeout → the client drops the transport and loses the response. **Fix (`ActorService::run`, `mcp-server/src/lib.rs`):** a non-auto phase-control `Prompt` now **acknowledges immediately** (`ok:true`, "submitted to the operator… call phase_status to confirm") and runs the operator approval + phase change in a **detached `tokio::spawn`** task — the HTTP call returns at once. `execute_and_audit` was extracted from a method to a free fn so the task can call it without `&self`. CC rediscovers the result via `phase_status` / the phase footer. Auto-phase ON still runs inline (fast, returns the real from→to). Non-phase prompts (trust-tier / danger-zone) still block (the agent needs the result). **Idempotent for free:** `PendingApprovals` is one-waiter-per-session and *supersedes* on re-register, so a retry after a "lost" response cancels the prior waiter — exactly one execution. +2 tests (ack-then-execute-on-approve; denied-out-of-band-never-executes); replaced the old `auto_phase_off_refuses…` test. Runtime-verify: agent calls `request_phase` → gets the ack instantly (no drop) → operator-approval prompt appears in the cockpit → on approve, `phase_status` shows the new phase.

## THREE QUICK WINS (interrupt-detect · space-tab dot · auto-phasing toggle) — DONE (2026-06-08) — green, 365 tests (+4), clippy clean (my files)
All collision-free (NOT `session_monitor.rs`).
- **QW1 — reliable interrupt detection (self-calibrating):** `crates/detection` now flags an `Assistant` turn that goes quiet with no following clean stop → `Incomplete` — **but only once this session has produced a clean stop before** (`FileState.seen_clean_stop`, set on `Turn::StopClean`), proving a `Stop` hook is active. No clean stop ever seen ⇒ fall back to the `User`-turn-only stall (zero false positives for hookless sessions). Operator has a Stop hook via OMC, so it's live for them; installing MoonlightCode's *own* Stop hook (for non-OMC users) is the only remaining bit, deferred (uncertain whether a no-op Stop hook emits `stop_hook_summary` + it mutates `~/.claude/settings.json`). +1 test.
- **QW2 — space-tab ⚠ dot:** `theme::attention_color` (shared; `session_tile` now uses it too). `workspace` folds `SessionAlert` into a `session_alerts` map; `space_attention` unifies status + overlay via `AttentionKind::severity` → the space-tab dot now lights for Stuck/Incomplete, not just Waiting/Errored.
- **QW3 — auto-phasing toggle:** a `⟳ auto-phase` toggle in the **main toolbar** flips a shared `Arc<AtomicBool>` (`ShellDeps.auto_phase`). The `ActorService` reads it (`with_auto_phase`): when on, a control-plane `Prompt` (`request_phase`) is **auto-approved** (audited `Approved · auto-phase`) instead of raising a cockpit hold — **PDP stays pure, no `Session`/`PermissionRequest` ripple**. Off by default; never affects any non-phase verb. +3 tests.
- ⚠️ **Runtime-verify:** (1) kill a CC mid-reply (with a Stop hook active) → ~3 min later ⚠ Incomplete on its tile + space-tab dot; (2) toggle `⟳ auto-phase` on → an agent `request_phase` applies with no cockpit prompt (audit shows `auto-phase`); off → it prompts as before.

## STUCK/INCOMPLETE SESSION WARNING SIGNAL — Slices 1+2+3 ALL DONE (2026-06-08) — green, 361 tests; PTY-exit + Assistant-mid-gen + space-tab dot remain as follow-ups
...
- **C2 (the Postman UI) — DONE (2026-06-08), 367 tests:** center **HTTP tab** (`panels/http_panel.rs`) — method (cycling pill) + URL input + Headers/Body editor tabs + **env switcher** (cycling pill; persists the choice to the manifest's `active`) + **Send** (interpolate `{{vars}}` from the active env → host-scope gate → `spawn_blocking(http::send)` → record into the **shared `HttpHistory`**, so operator + agent calls share one history) + **response pane** (status·ms·bytes colour-coded, Body/Headers tabs). **Saved requests:** `＋ Save` upserts into `.moonlight/http/requests.json`; a toggled `saved (N) ▾` list reloads one. **`[↗ open the request builder]`** row on the Services HTTP section emits the new `OpenRequest::Http` (dedup key `http`, one shared tab; registered for layout-restore). Data layer (C2a): `http.rs` gained `HttpOutcome.{status_text,headers,body}`, `SavedRequest`/`HttpRequests` + `load/save_requests`, `save_manifest` (+tests).
  - **Editor rework (2026-06-08, /frontend-design):** the raw Headers|Body **tabs** were replaced with a single scrollable **request-config pane** = an editable **Headers key/value table** (`KvRow` = two `InputState` cells + an enable ☑ + ✕ remove + ＋ add row) over a **Body** section with a **format selector** (None · Raw · JSON · XML · Form). Raw/XML edit free text; **Form** uses its own key/value table → `http::form_urlencode` (+test); **JSON** has a `Table | Raw JSON` sub-toggle — Table is an object-builder key/value table → `http::json_object_from_pairs` (values kept typed when valid JSON, else string; +test) — Raw is free text. The format auto-sets `Content-Type` unless the operator added one. `kv_table` generalized over a `Tbl{Headers,Form,Json,Vars}` enum (`rows`/`rows_mut`/`add_row`).
  - **Environment editor (2026-06-08):** the `env: X ▾` toolbar pill now opens an in-panel **env editor** (was a cycle button) — env chips (switch/edit) + `＋ New`, a name input, a **Variables** key/value table (`Tbl::Vars`), and an **allowed-hosts** editor (one per line). `Save env` upserts into `.moonlight/http/environments.json` (rename drops the old key) + makes it active (`http::save_manifest`); `Delete` removes it. Variables are then usable as `{{name}}` in URL/headers/body, and allowed-hosts widens the SSRF gate per env — all without hand-editing JSON. Helpers `section_label`/`format_chip`; the old `parse_header_lines`/`headers_to_lines` removed. ⚠️ Runtime-verify: ⚙ Services → HTTP → ↗ opens the tab; Send a `{{base_url}}/health` against a local env → response renders + the call appears in the Services HTTP history; a public host is refused unless in `allowed_hosts`.
