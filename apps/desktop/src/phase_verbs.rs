...
impl PhaseVerbExecutor {
    pub fn new(commands: UnboundedSender<Command>, inner: Arc<dyn VerbExecutor>) -> Self {
        Self { commands, inner }
    }
    ...
    fn request_phase(&self, session: &SessionId, payload: &str) -> Result<String, ControlError> {
        ...
        let (command, confirmation) = if wanted.is_empty() || wanted.eq_ignore_ascii_case("next") {
            (
                Command::AdvancePhase {
                    session: session.clone(),
                },
                "advancing to the next workflow phase".to_string(),
            )
            ...
            (
                ...
                format!("moving to the {} phase", phase.label()),
            )
        } else {
...
impl VerbExecutor for PhaseVerbExecutor {
    async fn execute(
        ...
        match verb {
            ...
            McpVerb::ReportBlocked => self.report_blocked(session, payload),
...
mod tests {
    ...
    fn next_and_empty_emit_advance_phase() {
        ...
        let ex = PhaseVerbExecutor::new(tx, Arc::new(EchoInner));
